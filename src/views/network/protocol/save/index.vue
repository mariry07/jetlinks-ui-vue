  ...
  >
    ...
    >
      ...
      <a-row v-if="selectProtocolType === 'script' ">
        ...
        <a-col :span="24">
          <a-form-item label="脚本" :labelCol="{ span: 3 }" :wrapperCol="{ span: 21 }">
            <AceEditor
              mode="javascript"
              theme="eclipse"
          ...
          <a-form-item key="provider" label="类名" >
            <a-input
              v-decorator="['configuration.provider', {
                ...
                initialValue:data.configuration?data.configuration.provider:''
              }]"
              placeholder="请输入类名"
            />
          </a-form-item>
              ...
              <a-col :span="14">
                <a-input
                  v-decorator="['configuration.location', {
                    ...
                    initialValue:data.configuration?data.configuration.location:''
                  }]"
                  placeholder="请上传文件地址"
                />
              </a-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async GetProviders () {
        const result = await apis.networkProtocol.GetProviders()
      ...
      Submit (e) {
        ...
        validateFields((err, fileds) => {
          if (!err) {
            // console.log('Received values of form: ', fileds)
            const subData = fileds.type === 'script' ? {
              ...fileds,
              configuration: {
                ...fileds.configuration,
                script: this.script,
                lang: fileds.type === 'script' ? 'js' : '',
                transport: fileds.configuration.transport.join(',')
              }
            } : {
              ...fileds
            }
