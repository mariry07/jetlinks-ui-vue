    ...
    <Card>
      ...
      <Table border :columns="columns12" :data="tableList" row-key="id">
       <template slot-scope="{ row, index }" slot="action">
          ...
          <Button type="primary" size="small" style="margin-right: 5px" @click="addChildren(row)">添加子机构</Button>
     ...
     <Modal ref="modalID" />
     <Drawer ref="drawerID" targetType="org"></Drawer>
     <userautz ref="userautzID" dimensionTypeId="org"/>
  </div>
...
userautz
    ...
    methods: {
      ...
      addChildren(row){
        ...
        this.$refs.modalID.title = `添加${copyRow.id}机构`
        this.$refs.modalID.formValidate.parentId = copyRow.id
        this.$refs.modalID.modalVisible = true
      },
      editFun(row){
        let copyRow = deepClone(row)
        this.$refs.modalID.formValidate = copyRow
        this.$refs.modalID.title = `编辑${copyRow.id}机构`
        this.$refs.modalID.modalVisible = true
      },
      autzFun(row){
        let copyRow = deepClone(row)
