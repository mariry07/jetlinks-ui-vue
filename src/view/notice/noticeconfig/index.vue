<template>
  <div>
      ...
      <Card>
        ...
        <Table border :columns="columns12" :data="tableList">
          ...
          <template slot-scope="{ row, index }" slot="action">
            ...
            <Button type="primary" size="small" @click="recordFun(row)">通知记录</Button>
          </template>
        </Table>
    ...
    <Record ref="recordID" :records="records"/>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      recordFun(row) {
        let copyRow = deepClone(row)
        this.recordConfig(10,0,copyRow.id)
        this.$refs.recordID.recordVisible = true
      },
     async recordConfig(pageSize,pageIndex,id){
        let res = await recordConfig(pageSize,pageIndex, id)
